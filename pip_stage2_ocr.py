"""
PIP Stage 2: OCR Reader
Extracts text values from the located control row regions.
"""
import cv2
import numpy as np
import easyocr
...
from pip_localizer_final import UILocalizer, ControlRowLocation, LayoutType
...
class OCRReader:
    ...
    Uses the known label positions to find value regions.
    """
    
    # Expected controls and their confirmed ground-truth states
    EXPECTED_CONTROLS = {
        "STRATEGY": "BALANCED",
        "PACE": "NEUTRAL",
        "TYRE": "SOFT",
        ...
        "NITRO": "0",
        "PIT": "BOX BOX",
        "DEFEND": None,  # UNKNOWN - no confirmed state
    }
    ...
    def read_control_row(self, frame: np.ndarray, 
        ...
        """
        Read all control values from the located control row.
        """
        readings = {}
        
        # Get the control row region
        ...
        # Run OCR on both enhanced and thresholded
        all_results = []
        ...
        # Keep all results for matching (don't deduplicate)
        ...
        # Now match expected controls to OCR results
        readings = {}
    ...
    def _match_control_value(self, control_name: str, expected_value: str,
        ...
        # Collect all candidate matches with their match type
        candidates = []
...
class PIPPerception:
    """
    Combined PIP perception pipeline: Localizer + OCR Reader
    """
...
def test_full_pipeline():
    """Test the full PIP pipeline on ground truth"""
    pip = PIPPerception()
